...
/* Static demo mode.
 *
 * The full app needs a Python server, a SQLite file it can write to, and a
 * Docker daemon to render inside -- none of which exist on a static host. But
 * everything worth *showing* is already finished: three sessions with their
 * diagnoses, beats, measured timings, chat transcripts and videos. So
 * `scripts/export_demo.py` writes those out as flat JSON and injects
 * `ASTRAY_DEMO` ahead of this file, and the two places that talk to a server
 * are redirected at them.
 *
 * There are exactly two such places, which is why this is a shim and not a
 * fork: every request goes through `api()`, and the only other one is the
 * EventSource further down -- guarded rather than rewritten, because a finished
 * session has nothing left to stream.
 *
 * Writes are refused rather than faked. A demo that silently accepted a new
 * problem and then never produced an animation would be worse than one that
 * says plainly what it is. */
const DEMO = Boolean(window.ASTRAY_DEMO);

const demoPath = (path) => {
  const [route] = path.split("?");
  if (route === "/api/insights") return "demo/insights.json";
  if (route === "/api/sessions") return "demo/sessions.json";
  const m = route.match(/^\/api\/sessions\/([^/]+)(?:\/(\w+))?$/);
  if (m) return `demo/${m[1]}/${m[2] || "session"}.json`;
  return path;
};
...
const api = (path, opts) => {
  if (DEMO) {
    if ((opts?.method || "GET").toUpperCase() !== "GET") {
      return Promise.reject(new Error("this is a read-only demo; run it locally to diagnose new work"));
    }
    path = demoPath(path);
  }
...
function renderSubmit() {
  ...
  /* Say what this build is before anything is typed, not after.
   *
   * The refusal in `api()` is the guarantee, but discovering it by filling in a
   * problem and pressing the button is a bad way to learn it. The message names
   * what still works, because in a demo build almost everything does. */
  if (DEMO) {
    // Reuses the error slot for position, not for tone. This is a notice about
    // the build, and styling it as a failure would read as something going wrong.
    err.className = "field-hint";
    err.textContent =
      "Read-only demo: the three sessions below are real runs, with their animations, "
      + "timings and tutor transcripts. Diagnosing new work needs the full stack — see the README.";
  }
...
function renderSession(sessionId) {
  ...
  // Nothing to stream in a demo build: every session in it already finished,
  // and opening an EventSource against a static host just retries forever.
  const es = DEMO ? null : new EventSource(`/api/sessions/${sessionId}/stream`);
